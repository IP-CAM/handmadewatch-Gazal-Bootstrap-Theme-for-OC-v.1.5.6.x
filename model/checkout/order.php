	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			foreach ($order_product_query->rows as $order_product) {
				...
				if ($product_data['sku'] == "") {
					$remeshok = $this->model_catalog_product->getRemeshok($order_product['product_id']);
					$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$remeshok['product_id'] . "' AND subtract = '1'");
					$tsiferblat = $this->model_catalog_product->getTsiferblat($order_product['product_id']);
					$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$tsiferblat['product_id'] . "' AND subtract = '1'");
