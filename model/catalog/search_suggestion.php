			...
			if (!empty($data['filter_name'])) {
				...
				foreach ($words as $word) {
					//if (!empty($data['filter_description'])) {
						$implode[] = "(LCASE(pd.name) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(pd.description) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(pd.tag) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(pd.meta_description) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(pd.meta_keyword) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(p.model) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(p.sku) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%' OR LCASE(pa.text) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%')";
					//} else {
