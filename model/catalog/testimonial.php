...
class ModelCatalogTestimonial extends Model {
	...
	public function getTestimonials($start = 0, $limit = 20, $random = true) {
		if ($random == false)
	...
	if ($this->db->escape($data['image'])) {
	$image = $this->encryption->decrypt($this->db->escape($data['image']));
		
		} elseif ($this->db->escape($data['sex']) == 2) {
		$image = "female.png";
		} else {
		$image = "male.png";
		}
	
		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET status = '".$status."', rating = '".$this->db->escape($data['rating'])."', name='".$this->db->escape($data['name'])."', city = '".$this->db->escape($data['city'])."', email='".$this->db->escape($data['email'])."', sex='".$this->db->escape($data['sex'])."', image='". $image ."', date_added = NOW()");
