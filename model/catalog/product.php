...
class ModelCatalogProduct extends Model {
	public function getSborka($product_id)
	{
	$product_info = $this->getProduct($product_id);
	if ($product_info['sku'] == "") {
			$remeshok = $this->getRemeshok($product_id);
			$tsiferblat = $this->getTsiferblat($product_id);
			$res['sku'] = $remeshok['sku'] . " / " . $tsiferblat['sku'];
			if ($remeshok['quantity'] > $tsiferblat['quantity']) {
				$res['quantity'] = $tsiferblat['quantity'];
			} else {
				$res['quantity'] = $remeshok['quantity'];
			}
		} else {
		$res['sku'] = $product_info['sku'];
		$res['quantity'] = $product_info['quantity'];
	}
	return $res;
}
	...
	public function getRemeshok($product_id) {
	
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute_description WHERE name = 'Тип ремешка'");
		if ($query->num_rows) {
			$tip_id = $query->row['attribute_id'];
		} else {
			return false;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_attribute WHERE product_id = '".$product_id."' AND attribute_id = '".$tip_id."'");
		if ($query->num_rows) {
			$tip_text = $query->row['text'];
		} else {
			return false;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product op LEFT JOIN " . DB_PREFIX . "product_attribute opa ON (op.product_id = opa.product_id)  WHERE NOT op.sku = '' AND opa.text = '".$tip_text."'");
		foreach ($query->rows as $result) {
			$tips[] = $result['product_id'];
		}	
					
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute_description WHERE name = 'Цвет ремешка'");
		if ($query->num_rows) {
			$tsvet_id = $query->row['attribute_id'];
		} else {
			return false;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_attribute WHERE product_id = '".$product_id."' AND attribute_id = '".$tsvet_id."'");
		if ($query->num_rows) {
			$tsvet_text = $query->row['text'];
		} else {
			return false;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product op LEFT JOIN " . DB_PREFIX . "product_attribute opa ON (op.product_id = opa.product_id)  WHERE NOT op.sku = '' AND opa.text = '".$tsvet_text."'");
		foreach ($query->rows as $result) {
			$tsvets[] = $result['product_id'];
		}
		if (count(array_intersect($tsvets,$tips)) == 1) {	
			$remeshok = array_intersect($tsvets,$tips);		
			foreach ($remeshok as $key => $value) {
			$remeshok = $value;
			}
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '".$remeshok."'");
	...
	public function getTsiferblat($product_id) {
	
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '".$product_id."'");
		$tip_text = $query->row['model'];		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE NOT sku = '' AND model = '".$tip_text."'");
		foreach ($query->rows as $result) {
			$tips[] = $result['product_id'];
		}	
					
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute_description WHERE name = 'Цвет циферблата'");
		if ($query->num_rows) {
			$tsvet_id = $query->row['attribute_id'];
		} else {
			return false;
		...
		if (count(array_intersect($tsvets,$tips)) == 1) {
			$tsiferblat = array_intersect($tsvets,$tips);		
			foreach ($tsiferblat as $key => $value) {
			$tsiferblat = $value;
			}
		} else {
			return false;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '".$tsiferblat."'");
	...
	public function getProducts($data = array()) {
		...
		$sql .= " AND (";
			...
			$i = 1;
			foreach ($data['filter_tags'] as $taggies) {
			
				$sql .= "pd.tag LIKE '%" . $this->db->escape($taggies) . "%'";
				if ($i != count($data['filter_tags'])) {
				$sql .= " OR ";
				}
				$i = $i + 1;
			}
