...
class ModelModuleRelatedOptions extends Model {
  ...
  public function getJournal2Price($product_id) {
	...
	public function get_related_options_set_by_poids($product_id, $options, $full_equal=false) {
	...
	public function getMinMaxRO_Price($product_id) {
		...
		if ($this->installed()) {
			...
			if ($prices) {
				...
				return array('product_price'=>$product_price, 'min'=>$min_price, 'max'=>$max_price, 'min_delta'=>$min_price-$product_price, 'max_delta'=>$max_price-$product_price );
			}
		}
		return false;
	}
