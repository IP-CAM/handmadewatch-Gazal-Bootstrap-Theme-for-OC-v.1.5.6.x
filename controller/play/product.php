            ...
            if($auth == true){
                ...
                if($products != 0){
                    foreach($products as $product){
						...
						if ($product['sku'] == "") {
			$remeshok = $this->model_catalog_product->getRemeshok('play_product_id');
			$tsiferblat = $this->model_catalog_product->getTsiferblat('play_product_id');
			$product['sku'] = $remeshok['sku'] . " / " . $tsiferblat['sku'];
			if ($remeshok['quantity'] > $tsiferblat['quantity']) {
				$product['quantity'] = $tsiferblat['quantity'];
			} else {
				$product['quantity'] = $remeshok['quantity'];
			}
		}
