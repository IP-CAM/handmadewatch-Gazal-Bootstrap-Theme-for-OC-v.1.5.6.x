...
class ControllerModuleCart extends Controller {
	public function index() {
		...
		if ($n > 20) {
		$des = floor(($n/100 - floor($n/100))*10);		
			if ($des != 1) {
				$n = ($n/10 - floor($n/10))*10;				
			} else {
			$n = 5;
			}
		}
		...
		$this->data['text_items'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $pref, $this->currency->format($total));		
		} else {
		$this->data['text_items'] = $this->language->get('text_empty');				
		}
